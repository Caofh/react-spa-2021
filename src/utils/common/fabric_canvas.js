...
/* canvas添加base64图片或网图(宽高自定义)（可以网图，也可以是base64的图）；注意***：图片的跨域问题，必须同域图片。才可使用canvas.toDataUrl()等api
  let options = {
    ...
    // base64: 'https://oemu.bytedance.net/files/default/2020/05-22/09502519b2db796526.jpg?version=4.1.84&format=240x240.jpeg',
    width: 80,
    height: 80,
    top: 670,
    left: 0,
  ...
  return new Promise((resolve, reject) => {
    fabric.util.loadImage(options.base64, (nodeImg) => {
      ...
      let config = {
        ...
        width: width,
        height: height,
        scaleX: options.width ? options.width / width : (options.height / height),
        scaleY: options.height ? options.height / height : (options.width / width),
        selectable: options.selectable || false, // 禁止circle被选中
      }
      var imgInstance = new fabric.Image(nodeImg, config);
...
/* canvas添加网络图片(宽高自定义)(只能是网图)；注意***：图片的跨域问题，必须同域图片。才可使用canvas.toDataUrl()等api
  let options = {
    imgUrl: 'https://oemu.bytedance.net/files/default/2020/05-22/09502519b2db796526.jpg?version=4.1.84&format=240x240.jpeg',
    width: 171,
    height: 171,
    top: 0,
    left: 0,
